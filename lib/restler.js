...
function Request(uri, options) {
  ...
  if (this.options.multipart) {
    ...
    if (typeof this.options.data == 'object') {
      // Send raw data
      if (this.options.data instanceof multipart.Data) {
        console.log('send raw data');
        this.headers['Content-Type'] = this.options.data.contentType;
        this.options.data = this.options.data.data;// Buffer
...
mixin(Request.prototype, {
  ...
  _responseHandler: function(response) {
    ...
    if (self._isRedirect(response) && self.options.followRedirects) {
      ...
      response.on('end', function() {
        console.log('raw encoded: ', util.inspect(body));
        response.rawEncoded = body = Buffer.concat(body);
        self._decode(body, response, function(err, body) {
          ...
          console.log('raw body: ', util.inspect(response.raw), 'iconv body: ', util.inspect(body));
  ...
  _encode: function(body, response, callback) {
    ...
    if (self.options.decoding == 'buffer') {
      ...
      console.log('body: ', util.inspect(body), 'decoding: ', self.options.decoding);
  ...
  json: function(data, callback) {
    if (data && data.length) {
      ...
      try {
        ...
        console.log('data: ', util.inspect(data), 'parsed: ', util.inspect(parsedData));
